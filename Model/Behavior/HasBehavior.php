...
class HasBehavior extends ModelBehavior {
    ...
    public function saveDiff(Model $Model, $data = array(), $options = array()) {
        ...
        if (isset($data[$Model->name][$Model->primaryKey]) && $data[$Model->name][$Model->primaryKey]) {
            ...
            $in =
                Set::extract("/{$hasManyName}/{$Model->{$hasManyName}->primaryKey}[{$Model->{$hasManyName}->primaryKey}>0]",
                    $data);
            ...
            if (!$Model->{$hasManyName}->deleteAll(array($hasManyName . "." .
            $Model->{$hasManyName}->primaryKey => $diff), true)
            ) {
                $commit = false;
            }
        }
